...
| `0.17.0` | Delete-relaxed reachability inside the lifted extractor: `RelaxedReachability` and its options/statistics/table types, plus `LiftedFactLandmarkGeneratorOptions.{reachability_filter_members, reachability_disambiguation, first_achievers_restricted, verify_pi_plus, complete_fact_landmarks}`, the `ReachabilityDisambiguation` and `CompleteFactLandmarks` enums, and `verify_pi_plus_fact_landmarks`. |
