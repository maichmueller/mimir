...
class CustomGoalCountHeuristic(IHeuristic):
    def __init__(self, problem: Problem, pddl_repositories: PDDLRepositories):
        ...
        self.num_goal_literals = len(self.problem.get_fluent_goal_condition()) + len(
            self.problem.get_derived_goal_condition()
        )
...
class CustomAStarAlgorithmEventHandler(AStarAlgorithmEventHandlerBase):
    def __init__(self, quiet=True):
        ...
        """
        AStarAlgorithmEventHandlerBase.__init__(
            self, quiet
        )  # Without this, a TypeError is raised.

    def on_expand_state_impl(
        self, state: State, problem: Problem, pddl_repositories: PDDLRepositories
    ):
        pass

    def on_generate_state_impl(
        self,
        state: State,
        action: GroundAction,
        problem: Problem,
    ...
    def on_generate_state_relaxed_impl(
        self,
        state: State,
        action: GroundAction,
        problem: Problem,
    ...
    def on_generate_state_not_relaxed_impl(
        self,
        state: State,
        action: GroundAction,
        problem: Problem,
    ...
    def on_close_state_impl(
        self, state: State, problem: Problem, pddl_repositories: PDDLRepositories
    ):
        pass

    def on_finish_f_layer_impl(
        self, f_value: float, num_expanded_states: int, num_generated_states: int
    ):
        pass

    def on_prune_state_impl(
        self, state: State, problem: Problem, pddl_repositories: PDDLRepositories
    ):
        pass

    def on_start_search_impl(
        self, start_state: State, problem: Problem, pddl_repositories: PDDLRepositories
    ):
        pass
    ...
    def on_solved_impl(
        self,
        ground_action_plan: List[GroundAction],
        pddl_repositories: PDDLRepositories,
    ):
        pass
...
def test_astar_search():
    ...
    goal_count_heuristic = CustomGoalCountHeuristic(
        parser.get_problem(), parser.get_pddl_repositories()
    )

    event_handler = CustomAStarAlgorithmEventHandler(False)
    astar_search_algorithm = AStarAlgorithm(
        lifted_applicable_action_generator,
        state_repository,
