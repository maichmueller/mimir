...
concept IsIncidenceGraph = requires(T a, VertexIndex vertex, EdgeIndex edge) {
    ...
    { a.template get_source<ForwardTraversal>(edge) } -> std::same_as<VertexIndex>;
    { a.template get_source<BackwardTraversal>(edge) } -> std::same_as<VertexIndex>;
    { a.template get_target<ForwardTraversal>(edge) } -> std::same_as<VertexIndex>;
    { a.template get_target<BackwardTraversal>(edge) } -> std::same_as<VertexIndex>;
    { a.template get_adjacent_edge_indices<ForwardTraversal>(vertex) } -> IsRangeOver<EdgeIndex>;
    { a.template get_adjacent_edge_indices<BackwardTraversal>(vertex) } -> IsRangeOver<EdgeIndex>;
    { a.template get_degree<ForwardTraversal>(vertex) } -> std::same_as<Degree>;
    { a.template get_degree<BackwardTraversal>(vertex) } -> std::same_as<Degree>;
};
...
concept IsAdjacencyGraph = requires(T a, VertexIndex vertex) {
    ...
    { a.template get_adjacent_vertex_indices<ForwardTraversal>(vertex) } -> IsRangeOver<VertexIndex>;
    { a.template get_adjacent_vertex_indices<BackwardTraversal>(vertex) } -> IsRangeOver<VertexIndex>;
};
