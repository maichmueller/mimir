...
std::optional<TupleVertexIndexList> TupleGraph::compute_admissible_chain(const StateList& states)
{
    ...
    const auto distances = m_state_space->compute_shortest_distances_from_states<ForwardTraversal>(IndexList { m_state_space->get_initial_state_index() });
    for (const auto& state : states)
    {
        const auto state_index = m_state_space->get_index(state);
...
void TupleGraphArityZeroComputation::compute_first_layer(State root_state)
{
    ...
    const auto root_state_index = m_state_space->get_index(root_state);
